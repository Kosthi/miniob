...
def __run_shell_command(command_args, env=None):
    '''
    ...
    outputs = []
    command_process = subprocess.Popen(command_args, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
    while True:
...
def compile(work_dir: str, build_dir: str, cmake_args: str, make_args: str, rebuild_all: bool, eval_result: EvalResult):
    ...
    # 在运行 make 命令之前，设置环境变量以使用正确的 Clang 编译器
    env = os.environ.copy()
    env["CC"] = "/opt/homebrew/opt/llvm/bin/clang"
    env["CXX"] = "/opt/homebrew/opt/llvm/bin/clang++"

    ret, outputs = __run_shell_command(make_command, env=env)
    if ret != 0:
